...
namespace Agent
{
     public class ParticleEmitterUpdateSystem
    {
        ...
        public ParticleEmitterUpdateSystem(Contexts entitasContext)
        {
            ...
            foreach (var gameEntity in entities)
            {
                ...
                if (state.CurrentTime <= 0.0f)
                {
                    for(int i = 0; i < state.ParticleCount; i++)
                    {
                        ...
                        e.AddParticle2dPosition(position.Position, state.ParticleAcceleration, new Vector2(state.ParticleStartingVelocity.x + randomX, state.ParticleStartingVelocity.y));
                    }
